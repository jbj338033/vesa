...
pub enum Message {
    ...
    PointerAxis { time: u32, axis: u8, value: f64 },
    KeyboardKey { time: u32, key: u32, state: u8 },
...
pub fn encode(msg: &Message) -> Vec<u8> {
    match *msg {
        ...
        Message::PointerButton { time, button, state } => {
        ...
        Message::KeyboardModifiers { depressed, latched, locked, group } => {
...
fn check_len(buf: &[u8], expected: usize) -> Result<(), DecodeError> {
    if buf.len() < expected {
        return Err(DecodeError::TooShort { expected, got: buf.len() });
    }
    Ok(())
}
...
pub fn decode(buf: &[u8]) -> Result<Message, DecodeError> {
    ...
    match buf[0] {
        ...
        TAG_POINTER_BUTTON => {
            ...
            Ok(Message::PointerButton { time, button, state })
        }
        TAG_POINTER_AXIS => {
            check_len(buf, 14)?;
            let time = u32::from_be_bytes([buf[1], buf[2], buf[3], buf[4]]);
        ...
        TAG_KEYBOARD_MODIFIERS => {
            ...
            Ok(Message::KeyboardModifiers { depressed, latched, locked, group })
        }
...
impl Message {
    pub fn from_input_event(event: &InputEvent) -> Self {
        match *event {
            InputEvent::PointerMotion { time, dx, dy } => {
                Message::PointerMotion { time, dx, dy }
            }
            InputEvent::PointerButton { time, button, state } => {
                Message::PointerButton { time, button, state: state.to_u32() }
            }
            InputEvent::PointerAxis { time, axis, value } => {
                Message::PointerAxis { time, axis: axis.to_u8(), value }
            }
            InputEvent::KeyboardKey { time, key, state } => {
                Message::KeyboardKey { time, key, state: state.to_u8() }
            }
    ...
    pub fn to_input_event(&self) -> Option<InputEvent> {
        match *self {
            ...
            Message::PointerButton { time, button, state } => {
                ...
                Some(InputEvent::PointerButton { time, button, state })
            }
            Message::PointerAxis { time, axis, value } => {
            ...
            Message::KeyboardModifiers { depressed, latched, locked, group } => {
                Some(InputEvent::KeyboardModifiers { depressed, latched, locked, group })
            }
            _ => None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn pointer_motion() {
        roundtrip(Message::PointerMotion { time: 1000, dx: -1.5, dy: 2.75 });
    }

    #[test]
    fn pointer_button() {
        roundtrip(Message::PointerButton { time: 500, button: 272, state: 1 });
    }

    #[test]
    fn pointer_axis() {
        roundtrip(Message::PointerAxis { time: 800, axis: 0, value: -15.0 });
    }

    #[test]
    fn keyboard_key() {
        roundtrip(Message::KeyboardKey { time: 1200, key: 30, state: 1 });
    }
    ...
    fn decode_unknown_tag() {
        assert!(matches!(decode(&[0xFF]), Err(DecodeError::UnknownTag(0xFF))));
    }

    #[test]
    fn decode_too_short() {
        assert!(matches!(decode(&[TAG_ACK, 0x00]), Err(DecodeError::TooShort { .. })));
    }

    #[test]
    fn input_event_roundtrip() {
        let event = InputEvent::PointerMotion { time: 100, dx: 1.0, dy: -1.0 };
        let msg = Message::from_input_event(&event);
        let back = msg.to_input_event().unwrap();
        match (event, back) {
            (InputEvent::PointerMotion { time: t1, dx: dx1, dy: dy1 },
             InputEvent::PointerMotion { time: t2, dx: dx2, dy: dy2 }) => {
                assert_eq!(t1, t2);
                assert_eq!(dx1, dx2);
                assert_eq!(dy1, dy2);
            }
    ...
    fn encoded_sizes() {
        ...
        assert_eq!(encode(&Message::PointerAxis { time: 0, axis: 0, value: 0.0 }).len(), 14);
        assert_eq!(encode(&Message::KeyboardKey { time: 0, key: 0, state: 0 }).len(), 10);
